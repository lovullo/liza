...
describe('DataApiMediator', () => {
  ...
  describe('updateFieldData event', () => {
    ...
    [
      {
        ...
        val_label: [{value: 'first result', label: 'first'}],
      ...
      {
        label:
          'uses first value of result if existing not in result set (first index)',
        name: 'foo',
        index: 0,
      ...
      {
        label:
          'uses first value of result if existing not in result set (second index)',
        name: 'foo',
        index: 1,
        value: {foo: ['does not', 'exist']},
        expected: {
      ...
      {
        label:
          'uses first value of result if existing not in result set (all indexes)',
        name: 'foo',
        index: -1,
        value: {foo: ['does not', 'exist']},
        expected: {
      ...
      ({
        ...
        it(label, done => {
          ...
          const sut = Sut(ui, {}, {[name]: dapimap}, getQuote).monitor(
            dapi_manager
          );

          dapi_manager.emit('updateFieldData', name, index, val_label, results);
    ...
    it('does not perform expansion if data are not available', done => {
      ...
      dapi_manager.getDataExpansion = () => {
        throw MissingDataError('this should happen, but should be caught');
      };
      ...
      const val_label = [{value: value, label: 'bar'}];

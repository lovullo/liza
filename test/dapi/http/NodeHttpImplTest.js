...
const {HttpImpl, NodeHttpImpl: Sut, HttpError} = require('../../../').dapi.http;
...
describe('NodeHttpImpl', () => {
  ...
  [
    ...
    it(label, done => {
      ...
      Sut({http: http, https: https}, url).requestData(
        '',
        method,
        data,
        callback
  ...
  describe('given an origin', () => {
    it('prepends to URL if URL begins with a slash', done => {
      ...
      Sut({http: http}, url, origin).requestData(path, 'GET', {}, () => {});
    });
  ...
  it('returns response when no error', done => {
    ...
    Sut({http: http}, _createMockUrl()).requestData(
      '',
      'GET',
      '',
      (e, data) => {
  ...
  it('adds data to query string on GET', done => {
    ...
    const http = _createMockHttp((options, callback) => {
      expect(options.path).to.equal(given_path + '?' + expected_query);

      callback(res);
      res.trigger('end');
    });

    Sut({http: http}, url).requestData('', 'GET', expected_query, done);
  });
  ...
  describe('protected API', () => {
    it('allows overriding request end behavior', done => {
      ...
      })({http: http}, _createMockUrl()).requestData(
        '',
        'GET',
        '',
        (given_e, given_data) => {
    ...
    it('allows overriding concept of success', done => {
      ...
      })({http: http}, _createMockUrl()).requestData('', 'GET', '', e => {
        expect(e).to.equal(null);
        done();
      });
    });
    ...
    it('allows overriding error handling', done => {
      ...
      Sut.extend({
        'override serveError'(given_e, given_res, given_data, callback) {

...
var dapi = require('../../../').dapi,
  ...
  dummy_url = 'http://foo',
  dummy_impl = Class.implement(dapi.http.HttpImpl).extend({
...
describe('HttpDataApi', function () {
  ...
  describe('.request', function () {
    ...
    [
      ...
      it(`${method} encodes (${i})`, () => {
        Sut(dummy_url, method, impl, enctype).request(data, _ => {});
    ...
    it('accepts null data, converting to empty string', function () {
      expect(function () {
        Sut(dummy_url, 'GET', impl).request(null, function () {
    ...
    it('rejects all other data types', function () {
      ...
      [123, Infinity, undefined, NaN, function () {}].forEach(function (data) {
        expect(function () {

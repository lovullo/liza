...
import {
  ...
  ContextStores,
  GroupContext as Sut,
} from '../../../src/ui/context/GroupContext';
...
describe('GroupContext', () => {
  ...
  [
    ...
    {
      label:
        'setOptions sets options on store when field context does not exist',
      fields: ['foo', 'bar'],
      field_to_set: 'bar',
      is_attached: false,
    ...
    ({
      ...
      it(label, () => {
        ...
        expect(context_set_options_is_called).to.equal(
          context_set_options_expected
        );
        expect(store_set_options_is_called).to.equal(
          store_set_options_expected
        );

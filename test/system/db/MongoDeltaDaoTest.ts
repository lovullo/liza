  ...
  describe('#getUnprocessedDocuments', () => {
    it('finds unprocessed documents that do not have rates pending', () => {
      ...
      const coll: MongoCollection = {
        ...
        createIndex() {},
        insert() {},
        find(selector, _fields, callback) {
          expect(selector['ratedata.__rate_pending']).to.deep.equal({
            $in: [[0], null],
          });
          expect(selector.deltaError).to.deep.equal({$ne: true});
          expect(selector.published).to.be.false;
          expect(selector.env).to.equal(stub_env);
    ...
    it('rejects with error on query failure', () => {
      ...
      const coll: MongoCollection = {
        ...
        createIndex() {},
        insert() {},
        find(_selector, _fields, callback) {
          find_called = true;

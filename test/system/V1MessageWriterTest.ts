...
import {AvroSchema, parse as avro_parse} from 'avro-js';
...
describe('system.V1MessageWriter', () => {
  it('Rejects improperly formatted data', () => {
    ...
    const result = new Sut(createMockEncoderCtor(schema), schema).write(
      ts,
      meta,
      delta,
      bucket,
  ...
  describe('#avroEncode parses', () => {
    [
      ...
      },
      {
        label: 'Boolean value',
        valid: true,
        delta_data: {foo: {array: [{boolean: true}]}},
      },
      {
        label: 'Simple string',
        valid: true,
        delta_data: {foo: {array: [{string: 'bar'}, {string: 'baz'}]}},
      },
      {
        label: 'Simple int',
        valid: true,
        delta_data: {foo: {array: [{double: 123}]}},
      },
      {
        label: 'Nested array',
        valid: true,
        delta_data: {foo: {array: [{array: [{string: 'bar'}]}]}},
      },
      {
        label: 'Array with nulls',
        valid: true,
        delta_data: {foo: {array: [{string: 'bar'}, {string: 'baz'}, null]}},
      },
      {
        label: 'Nested Array with mixed values',
        valid: true,
        delta_data: {
          foo: {array: [{array: [{string: 'bar'}, {double: 123321}, null]}]},
        },
      },
      {
        label: 'Non-array',
        ...
        expected: {
          invalid_data: 'bar',
          invalid_paths: ['delta', 'Data', 'bucket', 'foo'],
        },
      },
      {
        label: 'Map objects',
  ...
  describe('#setDataTypes annotates', () => {
    [
      ...
      },
      {
        label: 'Boolean Value',
        delta_data: {foo: [true]},
        expected: {foo: {array: [{boolean: true}]}},
      },
      {
        label: 'Simple string',
        delta_data: {foo: ['bar', 'baz']},
        expected: {foo: {array: [{string: 'bar'}, {string: 'baz'}]}},
      },
      {
        label: 'Simple int',
        delta_data: {foo: [123]},
        expected: {foo: {array: [{double: 123}]}},
      },
      {
        label: 'Nested array',
        delta_data: {foo: [['bar', 'baz']]},
        expected: {foo: {array: [{array: [{string: 'bar'}, {string: 'baz'}]}]}},
      },
      {
        label: 'Double nested array',
        delta_data: {foo: [[['bar', 123, null]]]},
        expected: {
          foo: {
            array: [{array: [{array: [{string: 'bar'}, {double: 123}, null]}]}],
          },
        },
      },
      {
        label: 'Array with nulls',
        delta_data: {foo: ['bar', 'baz', null]},
        expected: {foo: {array: [{string: 'bar'}, {string: 'baz'}, null]}},
      },
      {
        label: 'Nested Array with mixed values',
        delta_data: {foo: [['bar', 123321, null]]},
        expected: {
          foo: {array: [{array: [{string: 'bar'}, {double: 123321}, null]}]},
        },
      },
      ...
      it(label, () => {
        ...
        const sut = new Sut(avroEncoderCtr, stub_schema);
  ...
  it('Message is formatted correctly', () => {
    ...
    return expect(
      new Sut(encoder, schema).write(ts, meta, delta, bucket, ratedata)
    )
      .to.eventually.deep.equal(Buffer.from(''))
      .then(_ => {
...
function createMockEncoderCtor(
  stub_schema: AvroSchema

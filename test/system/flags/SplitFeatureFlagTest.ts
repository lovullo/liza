...
import { SplitFeatureFlag as Sut, SplitClient } from "../../../src/system/flags/SplitFeatureFlag";
import { FeatureFlag, FeatureFlagConditions } from '../../../src/system/flags/FeatureFlag';
...
describe( "SplitioFeatureFlag", () =>
{
    it( "isEnabled retrieves 'on' value and returns true", () =>
    {
        ...
        return expect( sut.isEnabled( 'liza_autosave' ) )
    ...
    it( "isEnabled uses defaults when the response is not on or off", () =>
    {
        ...
        return expect( sut.isEnabled( 'liza_autosave' ) )
                    .to.eventually.be.false
                    .then( () => expect( default_called ).to.be.true );
    } );

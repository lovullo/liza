...
const mkfail = function (name, arr) {
  return arr.map(function (value, i) {
    return value === undefined ? undefined : Failure(Field(name, i), value);
  });
};
...
describe('ValidStateMonitor', function () {
  describe('#update', function () {
    ...
    describe('given failures', function () {
          ...
          return new Promise(accept => {
            return Sut()
              .on('failure', function (failures) {
                expect(failures).to.deep.equal({foo: [fail[0], fail[1]]});
                accept();
              })
              .on('fix', nocall('fix'))
      ...
      it('marks failures with index gaps', function () {
        ...
        return mkstore({}).then(empty => {
          return new Promise(accept => {
            Sut()
              .on('failure', function (failures) {
                expect(failures).to.deep.equal({foo: [undefined, fail[1]]});
                accept();
              })
              .on('fix', nocall('fix'))
              .update(empty, {foo: fail});
        ...
        return new Promise((accept, reject) => {
          return mkstore({})
            .then(empty => {
              return Sut()
                .on('failure', function (failures) {
                  expect(failures).to.deep.equal({foo: fail});
      ...
      it('does not discard existing failures', function () {
        ...
        // has both failures
        var fail1 = Failure(Field('foo', 0), '', [
          Field('cause1', 0),
          Field('cause2', 0),
        ]);

        // has only one of the two failures
        var fail2 = Failure(Field('foo', 1), '', [Field('cause2', 1)]);
                  ...
                  return mkstore(update).then(store => {
                    return sut
                      .once('fix', function (fixed) {
                        expect(fixed).to.deep.equal({foo: ['causefix1']});
    ...
    describe('given data with absence of failure', function () {
      it('removes non-failures if field is present', function () {
        ...
        return new Promise((accept, reject) => {
          return mkstore({foo: ['bardata', 'baz']})
            .then(data => {
              return sut
                .on('fix', function (fixed) {
                  expect(fixed).to.deep.equal({foo: ['bardata']});
                  accept();
                })
      ...
      it('keeps failures if field is missing', function () {
        ...
        return new Promise((accept, reject) => {
          return mkstore({bar: ['baz', 'quux']})
            .then(data => {
              return Sut()
                .on('fix', function (fixed) {
                  expect(fixed).to.deep.equal({bar: ['baz', 'quux']});
                  accept();
                })
                .update(data, {
      ...
      it('keeps past failures on key if failure does not reoccur', () => {
        ...
        return mkstore({foo: [undefined, undefined]}).then(data =>
          Sut()
      ...
      describe('given a cause', function () {
        it('considers when recognizing fix', function () {
          ...
          return new Promise((accept, reject) => {
            return mkstore({cause: ['bar']})
              .then(data => {
                return Sut()
                  .on('fix', function (fixed) {
                    expect(fixed).to.deep.equal({foo: ['bar']});
          ...
          return new Promise((accept, reject) => {
            return mkstore(update_data)
              .then(data => {
                return Sut()
                  .on('fix', function (fixed) {
                    expect(fixed).to.deep.equal({foo: [1, 1]});

                    accept();
                  })
                  .update(data, {
                    foo: [
                      Failure(Field('foo', 0), '', [Field('cause', 0)]),
                      Failure(Field('foo', 1), '', [Field('cause', 1)]),
                    ],
                  })
                  .then(sut => {
                    return sut.update(data, {});
        ...
        it('considers any number of causes', function () {
          ...
          const fail = Failure(field, 'reason', [cause1, cause2]);
  ...
  describe('#getFailures', function () {
    it('is empty when no failures exist', function () {
      expect(Sut().getFailures()).to.deep.equal({});
    });
  ...
  describe('#clearFailures', () => {
      ...
      return new Promise((accept, reject) => {
        mkstore({})
          .then(empty => {
            ...
            return sut
              .on('fix', fixed => {
                expect(fixed).to.deep.equal({foo: [null]});
    ...
    it('does not error on non-existent failure', () => {
      expect(() => Sut().clearFailures(['foo', 'baz'])).to.not.throw(Error);
    });
  });
});

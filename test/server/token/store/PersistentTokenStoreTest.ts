...
import {PersistentTokenStore as Sut} from '../../../../src/server/token/store/PersistentTokenStore';
...
describe('PersistentTokenStore', () => {
  ...
  describe('#lookupToken', () => {
    ...
    (<[string, TokenData, Token<TokenState>][]>[
      ...
      it(label, () => {
        ...
        return expect(
          new Sut(dao, doc_id, ns, voidIdgen).lookupToken(token_id)
  ...
  describe('#createToken', () => {
    ...
    (<[string, TokenData, Token<TokenState>][]>[
      ...
      it(label, () => {
        ...
        return expect(
          new Sut(dao, doc_id, ns, () => token_id).createToken()
        ).to.eventually.deep.equal(expected);
      })
    );
  });
  ...
  ]).forEach(([method, token, data, expected]) =>
    describe(`#${method}`, () => {
      ...
      it('changes token state', () => {
        ...
        return expect(
          new Sut(dao, doc_id, ns, voidIdgen)[method](<any>token, data)
        ).to.eventually.deep.equal(expected);
      });
    })
  );

...
describe('DataProcessor', () => {
  [
    ...
    const {request, program, sut, quote} = createSutFromStubs(
      internal,
      internals
    );
  ...
  [
    ...
    it(label, () => {
      const actual = sut.processDiff(new_data, request, program, bucket, quote);
  ...
  it('passes data to bucket filter', () => {
    ...
    const filter = {
      ...
      filterValues(values: string[], _filter: string, _permit_null: boolean) {
        return values;
      },
    };
  ...
  it('invokes dapi manager when monitored bucket value changes', () => {
    ...
    const {request, program, filter, quote} = createStubs(false);
    ...
    for (let dapi_name in expected) {
      ...
      for (let i in expected_call) {
        ...
        let [gfield, gindex, gdapi_manager, gdapi, gdata] = triggered[
          dapi_name
        ][i];
  ...
  it('check _mapDapiData default values', () => {
    ...
    const {dapis} = sut.processDiff(data, request, program, bucket, quote);
...
function createStubDataApiContructor() {
  return (_apis: any, _request: UserRequest, _quote: ServerSideQuote) => {
    return createStubDataApiManager();
  };
}
...
function createStubDataApiManager() {
  return {
    ...
    hasFieldData(_name: string, _index: PositiveInteger) {
      return true;
    },
    ...
    clearPendingApiCall(_id: string) {
      return this;
    },
...
function createStubStagingBucket(bucket: any): StagingBucket {
  ...
  return <StagingBucket>(<unknown>{
    setCommittedValues(_data: Record<string, any>) {
      return this;
    },
    ...
    overwriteValues(_data: Record<string, any>) {
      return this;
    },
    ...
    getFilledDiff() {
      return bucket.data || {foo: 'Bar'};
    },

    revert(_evented?: boolean) {
      return this;
    },

    commit(_store?: {old: Record<string, any>}) {
      return this;
    },
    ...
    hasIndex(_name: string, _i: PositiveInteger) {
      return true;
    },

...
/* eslint no-var: "off", block-scoped-var: "off", node/no-deprecated-api: "off", no-redeclare: "off", prefer-arrow-callback: "off" */
/**
 ...
 */
module.exports = Class('QuoteDataBucketCipher').extend(EventEmitter, {
  /**
  ...
  'public encrypt': function (bucket, callback) {
    ...
    this._encryptFields(bucket.getData(), this._fields.length - 1, callback);
  ...
  'public decrypt': function (bucket, callback) {
    ...
    this._decryptFields(bucket.getData(), this._fields.length - 1, callback);
  ...
  'private _decryptFields': function (data, i, callback) {
    ...
    try {
      var enc_data = data[field] ? new Buffer(data[field][0], 'base64') : {};
    } catch (e) {

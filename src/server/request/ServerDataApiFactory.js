...
const {
  dapi: {
    DataApiFactory,
    http: {NodeHttpImpl, SpoofedNodeHttpImpl},
  },
  store: {StoreMissError},
  server: {
    dapi: {
 ...
 */
module.exports = Class('ServerDataApiFactory').extend(DataApiFactory, {
  /**
   * Origin URL
   * @type {string}
   */
  ...
  'override protected createDataApi'(type, desc, bucket) {
    return new TokenedDataApi(
      ...
      token_ns =>
        new PersistentTokenStore(this._tokdao, this._doc_id, token_ns, () =>
  ...
  'override protected descLookup'(api_name, desc) {
    return this._conf
      ...
      .then(desc_lookup =>
        desc_lookup.reduce((ret, value, key) => {
          // merges the two, with lookup taking precedence
          ret[key] = value;
          return ret;
        }, Object.create(desc))
      )
      .catch(e => (Class.isA(StoreMissError, e) ? desc : Promise.reject(e)));
  },

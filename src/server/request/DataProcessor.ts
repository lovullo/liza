...
import {
  StagingBucket,
  StagingBucketConstructor,
} from '../../bucket/StagingBucket';
...
export class DataProcessor {
   ...
   * @param program - active program
   */
  private _cleanInternals(data: Record<string, any>, program: any): void {
  ...
  private _triggerDapis(
    ...
    const dapis = Object.keys(dapi_fields)
      .map(field => {
  ...
  private _determineDapiFields(
    ...
    return Object.keys(mapis).reduce((result: any, src_field: string) => {
  ...
  private _mapDapiData(
    ...
    return Object.keys(mapsrc).reduce((result: any, srcid: any) => {
      const bucketid = mapsrc[srcid];

      const bdata =
        (diff_data[bucketid] || [])[index] ||

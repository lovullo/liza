...
/* eslint no-var: "off", no-unused-vars: "off", eqeqeq: "off", prefer-arrow-callback: "off" */
/**
...
module.exports = Class('DslRaterContext').extend(EventEmitter, {
  /**
  ...
  'public complete': function () {
    ...
    this._avail_count = this.processCompleted(this._results, this._avail_count);
      ...
      for (var chk_i in chk) {
        ...
        if (
          ...
          chk_results.forEachResult(function (result) {
            ...
            result.ineligible = 'Cannot display with ' + 'carrier ' + name;
    ...
    for (var cname in this._has_g_submit) {
      this._results.forEach(function (set) {
        set.forEachResult(function (result) {
          // TODO: class desc
          result.submit += (result.submit ? '; ' : '') + cname;
  ...
  'private _emitResults': function () {
    this.emit('complete', this.postProcessResults(this._results));
  },

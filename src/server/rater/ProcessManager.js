...
/* eslint no-var: "off", eqeqeq: "off", prefer-const: "off" */
/**
...
module.exports = Class('ProcessManager', {
  ...
  'public init'(logc, errc) {
    ...
    this._child.on('exit', (excode, sig) => {
      ...
      if (excode !== 0) {
        errc(
          'Rater process exited with error code ' +
            excode +
            (sig ? ' (' + sig + ')' : '')
        );
  ...
  'public byId'(id) {
    ...
    return {
      rate: (quote, session, indv, success, failure) => {
  ...
  'private _createRqid'(qid, indv) {
    return qid + '_' + (indv ? indv + '_' : '') + new Date().getTime();
  },
});

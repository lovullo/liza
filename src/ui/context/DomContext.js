...
/* eslint no-var: "off", no-undef: "off", prefer-arrow-callback: "off", no-unused-vars: "off" */
/**
 * DOM subset context
 *
...
module.exports = Class('DomContext')
  ...
  .extend(EventEmitter, {
    ...
    'public split': function (on_id, c) {
      ...
      this._getElementById(on_id, function (element) {
        ...
        c(
          element
            ? module
                .exports(element, _self._fieldFactory, inst, _self._fieldCache)
                .on('error', function (e) {
                  // "bubble up" errors
                  _self.emit('error', e);
                })
    ...
    'private _fromCache': function (name, index, lookup) {
      var data = (this._fieldCache[name] = this._fieldCache[name] || []);
      ...
      var _self = this;
      return (data[index] = this._fieldFactory
        .create(
          name,
          index,

...
/* eslint no-var: "off", prefer-arrow-callback: "off", no-undef: "off" */
/**
...
module.exports = Class('StepUiBuilder').extend(EventEmitter, {
  /**
  ...
  'private _processData': function (data, ui) {
    ...
    if (!data.content.html) {
      ...
      data.content.html =
        '<h1>Error</h1><p>A problem was encountered ' +
    ...
    ui.setContent(
      $('<div class="step-groups" />').append($(data.content.html))[0]
    );
  ...
  'private _createGroups': function (ui) {
    ...
    var $groups = $content.find('.stepGroup').each(function () {
      ...
      step.addExclusiveFieldNames(group.getGroup().getExclusiveFieldNames());

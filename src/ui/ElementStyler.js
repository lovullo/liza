...
/* eslint no-var: "off", no-undef: "off", no-unused-vars: "off", no-case-declarations: "off" */
/**
...
module.exports = Class('ElementStyler', {
  ...
  _answerStyles: {
    ...
    includeExclude: function (value, _, default_val) {
      ...
      return +value === 0 ? 'Excluded' : 'Included';
    },
    ...
    waitable: function (value) {
      return value.replace(/Please wait.../, '<div class="plswait">$&</div>');
    },
  },
  ...
  'public setOptions': function (name, index, options, val, $context) {
    ...
    for (var item in options) {
      ...
      $element.append($('<option>').attr('value', opt_value).text(opt.label));
    ...
    switch (this._getElementType(name)) {
      ...
      case 'checkbox':
        ...
        if ($context && $context.singleIndex) {
          // get all elements
          elements = this.getElementByName(name, undefined, null, $context);
        } else {
          var group_length = this.getElementLength(name, $context),
            current = index * group_length,
            end = (index + 1) * group_length - 1;
          while (current <= end) {
            elements.push(this.getWidgetByName(name, current, null, $context));
        ...
        while (i--) {
          ...
          const question =
            elements[i] instanceof jQuery ? elements[i][0] : elements[i];
      ...
      default:
        const $element = this.getElementByName(name, index, null, $context);
        $element.val('' + value);
    }
  ...
  styleAnswer: function (ref_id, value) {
    ...
    var format = this._getAnswerStyler(type),
      default_val = this._displayDefaults[ref_id] || this._defaults[ref_id];
  ...
  remove: function (id) {
    ...
    var $element = id instanceof jQuery ? id : $('#' + id);
  ...
  getIdElementFromName: function (name, index, $parent) {
    ...
    var $element = $parent.find("[name='" + name + "[]']:nth(" + index + ')');
  ...
  disable: function (name, value) {
    var $elements = name instanceof jQuery ? name : this.getElementByName(name);
  ...
  'public getElementByName': function (name, index, filter, $context) {
    ...
    var result = this.getWidgetByName(name, proper_index, filter, $context);
   ...
   * @return {jQuery} matches
   */
  'public getElementByNameLax': function (name, index, filter, context) {
    context = context || this._$context;
    ...
    return this.getElementByName(name, index, filter, context);
  },
  ...
  'public getElementIdFromName': function (name, index) {
    return this.getElementByName(name, index).attr('id') || '';
  },
  ...
  'public getProperIndex': function (name, index, $context) {
    ...
    // element length
    var proper_index = index !== undefined ? +index * len : undefined;
  ...
  'public getElementLength': function (name, $context) {
    ...
    switch (this._getElementType(name)) {
      ...
      case 'radio':
        return this.getWidgetByName(name, 0, null, $context).attr(
  ...
  'public isAField': function (name) {
    ...
    return (
      this._getElementType(name) !== undefined ||

...
var ProgressDialog = Class.extend({
  ...
  __construct: function (title, width, height) {
    ...
    this.$progressBar = $(
      '<div id="dialogProgressBar" class="progressBar"></div>'
    );

    this.$dialog.append(this.$content).append(this.$progressBar.hide());
  },
  ...
  addError: function (html) {
    this.$content.append(
      '<div class="error-box center bold">' + html + '</div>'
    );
  },

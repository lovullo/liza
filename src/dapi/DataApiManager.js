...
/* eslint no-var: "off", prefer-arrow-callback: "off", block-scoped-var: "off", no-redeclare: "off" */
/**
 ...
 */
module.exports = Class('DataApiManager').extend(EventEmitter, {
  /**
  ...
  'public getApiData': function (api, data, callback, name, index, bucket, fc) {
    var id = name === undefined ? new Date().getTime() : name + '_' + index;
    ...
    var apip =
      ...
      (function () {
        ...
        return (_self._dataApis[api] = _self._dataApiFactory
          .fromType(apidesc.type, apidesc, bucket, api)
          .then(api => api.on('error', e => _self.emit('error', e))));
      })();
  ...
  'public setFieldData': function (name, index, data, value, label, unchanged) {
    ...
    for (var i in data) {
      ...
      if (fdata[data_value]) {
        this.emit(
          'error',
          Error('Value is not unique for ' + name + ': ' + data_value)
        );
      }
  ...
  'public triggerFieldUpdate': function (name, index, value, label, unchanged) {
    var fdata = this._fieldData[name][index],
    ...
    if (!data || !fdata) {
      ...
      this.emit(
        'error',
        Error('updateFieldData missing data for ' + name + '[' + index + ']')
      );
    }
    ...
    if (fdepth[index] > 0) {
      ...
      if (!unchanged) {
        ...
        this.emit(
          'error',
          RangeError('updateFieldData recursion on ' + name + '[' + index + ']')
        );
      }
    ...
    try {
      ...
      this.emit('error', Error('updateFieldData hook error: ' + e.message));
    }
  ...
  'public hasFieldData': function (name, index) {
    ...
    return (this._fieldData[name] || {})[index] ? true : false;
  },
  ...
  'public getDataExpansion': function (
    ...
    } else if (field_data && field_data.___empty && field_value !== '') {
    ...
    for (var field in map) {
      ...
      fdata[index] = data[param] !== undefined ? data[param] : '';

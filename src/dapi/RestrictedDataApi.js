  ...
  .extend(EventEmitter, {
    ...
    __construct: function (data_api, desc) {
      if (!Class.isA(DataApi, data_api)) {
        throw Error('Expected object of type DataApi; given: ' + data_api);
      }
    ...
    'private _requestParamCheck': function (data) {
      ...
      for (var name in this._params) {
        ...
        if (def_data.type !== 'string') {
          throw Error('Only string param defaults are currently supported');
        }
      ...
      while (i--) {
        ...
        if (err.length > 0) {
          this.emit(
            'error',
            TypeError(
              'Return data missing param ' + param + ': ' + err.join(', ')
            ),
            callback,
            response
          );

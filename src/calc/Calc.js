...
exports.range = function (data, value) {
  ...
  return range_from < range_to ? result : result.reverse();
};
...
exports.length = function (data) {
  ...
  while (true) {
    ...
    var len = item[item.length - 1] === null ? item.length - 1 : item.length;
...
exports.month = function (data) {
  ...
  for (var i = 0; i < len; i++) {
    result.push(
      new Date(Date.parse(data[i].replace(/-/g, '/'))).getMonth() + 1
    );
  }
...
exports.year = function (data) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    result.push(new Date(Date.parse(data[i].replace(/-/g, '/'))).getFullYear());
  }
...
exports.isIn = function (data, value) {
  ...
  for (var v_key = 0; v_key < value.length; v_key++) {
    ...
    all_values_found = value_found ? true : all_values_found;
  }

  return all_values_found === true ? ['1'] : ['0'];
};

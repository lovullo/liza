...
/* eslint no-var: "off", prefer-arrow-callback: "off", no-unused-vars: "off", no-undef: "off" */
/**
 * Export client
 *
...
module.exports = Class('ExportClient', {
  ...
  'public export': function (quote, params, callback) {
    ...
    this._startExport(quote, params, function (e, response) {
      ...
      if (!token_id) {
        callback(Error('No token id provided for export request'), response);
      }
  ...
  'private _createExportApi': function (uri, pred) {
    ...
    return HttpDataApi.use(JsonResponse).use(AutoRetry(pred, tries, delay))(
      uri,
      'GET',
  ...
  'private _shouldTryAgain': function (status, error) {
    return status === 503 && error === 'EAGAIN';
  },
});

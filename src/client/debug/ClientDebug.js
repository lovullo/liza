...
module.exports = Class('ClientDebug', {
  ...
  __construct: function (client, storage) {
    ...
    this._dialog = ClientDebugDialog(client, staging)
      .addTab(
        require('./BucketClientDebugTab')().on('fieldOverlayToggle', function (
          value
        ) {
      ...
      .addTab(
        require('./ClassifyClientDebugTab')().on(
          'classifyNoHideToggle',
          function (value) {
  ...
  'public bgInit': function () {
    ...
    if (this._hasAutoloadFlag()) {
      this._dialog.show(false).setAutoloadStatus(true);
    }
  ...
  'public toggle': function () {
    this._dialog.toggle().setErrorDebugStatus(this.hasErrorDebugFlag());

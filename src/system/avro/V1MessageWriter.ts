...
export class V1MessageWriter implements MessageWriter {
  ...
  write(
    ...
    const avro_object = this._avroFormat(ts, meta, delta, bucket, ratedata);
  ...
  private _avroFormat(
    ...
    const exp_date_ms = {long: meta.expDate};
    const ts_ms = ts * 1000;
    ...
    switch (typeof data) {
      case 'object':
        if (data == null) {
          ...
          data_formatted = top_level ? arr : {array: arr};
        } else {
          ...
          data_formatted = top_level ? datum_formatted : {map: datum_formatted};
        }
        break;

  ...
  getUnprocessedDocuments(): Promise<DeltaDocument[]> {
    return new Promise((resolve, reject) => {
      ...
      this._collection.find(
        ...
        (e, cursor) => {
          if (e) {
            reject(new DaoError('Error fetching unprocessed documents: ' + e));
            return;
          }

          cursor.toArray((e: Error, data: DeltaDocument[]) => {
            if (e) {
              reject(new DaoError('Error fetching array from cursor: ' + e));
              return;
            }
    ...
    return new Promise((resolve, reject) => {
      ...
      this._collection.update(
        ...
        e => {
          if (e) {
            reject(
              context(new DaoError('Error updating published ts: ' + e), {
                doc_id: doc_id,
                quote_id: doc_id,
              })
            );
      ...
      this._collection.update(
        ...
        e => {
          if (e) {
            reject(
              context(
                new DaoError('Error marking document as processed: ' + e),
                {
                  doc_id: doc_id,
                  quote_id: doc_id,
    ...
    return new Promise((resolve, reject) => {
      this._collection.update(
        ...
        e => {
          if (e) {
            reject(
              context(new DaoError('Failed setting error flag: ' + e), {
                doc_id: doc_id,
                quote_id: doc_id,
              })
            );
  ...
  getErrorCount(): Promise<number> {
    return new Promise((resolve, reject) => {
      this._collection.find(
        ...
        (e, cursor) => {
          if (e) {
            reject(new Error('Failed getting error count: ' + e));
            return;
          }
          ...
          cursor.toArray((e: NullableError, data: any[]) => {
            if (e) {
              reject(
                context(new DaoError('Failed getting error count: ' + e), {
                  cursor: cursor,
                })
              );
              return;

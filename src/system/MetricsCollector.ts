...
export type MetricTimer = (_start_time?: [number, number]) => [number, number];
...
export class MetricsCollector {
  ...
  private hookMetrics(): void {
    this._emitter.on('delta-process-start', (uid: string) => {
      this._timing_map[uid] = this._timer();
    });

    this._emitter.on('delta-process-end', (uid: string) => {
  ...
  private getPushCallback(self: MetricsCollector): () => void {
    return (error?: Error | undefined, _response?: any, _body?: any): void => {
      if (error) {
